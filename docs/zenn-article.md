...
今回のデモは、テレビ型UIの基盤部分にあたる。具体的には以下を実装した。
...
選択した都道府県から気象庁の6桁エリアコードを自動で引き当て、天気予報とアラートの両方に使いまわす。住所入力も位置情報も不要にした。「迷わず設定できる」ことが、このアプリでは機能より優先される。
